...
export default {
  ...
  setup() {
    ...
    const options = {
      ...
      api_prod: ref(`https://pickle-rick-back-production.up.railway.app/api/v1/character`),
      comments: ref([]),
  ...
  methods: {
   ...
   try {
    ...
    async getComments(id) {
      const res = await this.axios.get(`${this.api_prod}/${id}/comments`);
    ...
    async postComment(id){
      ...
      try {
        const res = await this.axios.post(`${this.api_prod}/${id}/comments`,data);
        if(res.status == 201){
          this.author = ""
          this.comment = ""
        }
